...
public class PizzaControls1 : MonoBehaviour
{
    // Input variables
    ...
    public GameObject target_phrase;
    ...
    private int state = 0;
    private int slice_sel = -1;
    private int letter_sel = -1;
    ...
    private void Update()
    {
        if (state == 0)
        {
            slice_sel = get_joystick_sel(true);
            if (slice_sel >= 0)
            {
                state++;
                if (tick_limit >= 4)
        ...
        else if (state == 2)
        {
            if (get_joystick_angle() >= 0)
            {
                letter_sel = get_joystick_sel(false);
                if (get_char().Equals("<"))
                    backspace();
                else
                    add_char(get_char());
        ...
        UI.transform.GetChild(6).GetComponent<UnityEngine.UI.Text>().text = "" + state;
        ...
        while (i < 7)
        {
            ...
            if (i == slice_sel)
            {
                ...
                for (int j = 0; j < 4; j++)
                {
                    ...
                    if (j == letter_sel && get_joystick_angle() >= 0)
    ...
    void A()
    {
        target_phrase.GetComponent<TestPhrases>().next = true;
        text_box = "";
    }
    ...
    int get_joystick_sel(bool left)
    {
        if (left)
        {
            return (int)(get_joystick_angle() * 7);
        }
        else
        {
            float r_angle = get_joystick_angle() + 0.125f;
    ...
    string get_char()
    {
        return letters[slice_sel, letter_sel];
    }
